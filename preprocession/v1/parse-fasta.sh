...
# create temp dir with name of the original fasta 
...
for seqfile in ${OUTPUT_DIR}/${NAME}/*.fasta; do
    ...
    docker exec --interactive --tty kmc_test ./kmc -v -k${K} -cs${MAX_CNT} -fa ${seqfile} ${seqfile}_db . &&
    docker exec --interactive --tty kmc_test ./kmc_dump ${seqfile}_db ${seqfile}_db_dumped &&
